...
import { fetchData, CreateAccount, deleteAccounts, UpdateAccount } from 'src/store/apps/accounts'
...
const defaultColumns: GridColDef[] = [
  ...
  },
  {
    ...
    minWidth: 300,
    field: 'link',
    headerName: 'Website or App Store Link',
    renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{`${row.link}`}</Typography>
  },
  {
    flex: 0.1,
    minWidth: 90,
    field: 'bmid',
    headerName: 'BM ID',
    renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{`${row.bmId}`}</Typography>
  },
  {
    flex: 0.3,
    minWidth: 300,
    field: 'facebook1',
    headerName: 'Facebook Page Link1',
    renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{`${row.facebookOne}`}</Typography>
  },
  {
    flex: 0.3,
    minWidth: 300,
    field: 'facebook2',
    headerName: 'Facebook Page Link2',
    renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{`${row.facebookTwo}`}</Typography>
  },
  // {
  //   flex: 0.2,
  //   minWidth: 125,
...
const AdAccounts = () => {
  ...
  const [updateAcc, setUpdateAcc] = useState<any>({
    _id: '',
    name: '',
    currency: '',
    status: '',
  ...
  const editAccounts = (acc: any) => {
    setUpdateAcc({
      ...
      link: acc.link,
      facebookOne: acc.facebookOne,
      facebookTwo: acc.facebookTwo
    })
    setShowEdit(true)
  }
      ...
      renderCell: ({ row }: CellType) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Tooltip title='Edit Ad Acc'>
            <IconButton size='small' onClick={() => editAccounts(row)}>
              ...
              <Grid container spacing={6}>
                <Grid item sm={4} xs={12}>
                  <Box
                    ...
                    sx={{
                      ...
                      borderRadius: 1,
                      cursor: 'pointer',
                      ...(updateAcc.platform === 'meta'
                        ? { ...bgColors.primaryLight }
                        : { backgroundColor: 'action.hover' }),
                      border: theme =>
                        `1px solid ${
                          updateAcc.platform === 'meta' ? theme.palette.primary.main : theme.palette.divider
                        }`
                    }}
                  >
                    <Box sx={{ mb: 1, display: 'flex', alignItems: 'center', '& svg': { mr: 2 } }}>
                      ...
                      <Typography
                        variant='h6'
                        sx={{ ...(updateAcc.platform === 'meta' ? { color: 'primary.main' } : {}) }}
                      >
                        Meta
                      </Typography>
                    </Box>
                ...
                <Grid item sm={4} xs={12}>
                  <Box
                    ...
                    sx={{
                      ...
                      borderRadius: 1,
                      cursor: 'pointer',
                      ...(updateAcc.platform === 'google'
                        ? { ...bgColors.primaryLight }
                        : { backgroundColor: 'action.hover' }),
                      border: theme =>
                        `1px solid ${
                          updateAcc.platform === 'google' ? theme.palette.primary.main : theme.palette.divider
                        }`
                    }}
                  >
                    <Box sx={{ mb: 1, display: 'flex', alignItems: 'center', '& svg': { mr: 2 } }}>
                      ...
                      <Typography
                        variant='h6'
                        sx={{ ...(updateAcc.platform === 'google' ? { color: 'primary.main' } : {}) }}
                      >
                        Google Ads
                      </Typography>
                    </Box>
                ...
                <Grid item sm={2} xs={12}>
                  <Box
                    ...
                    sx={{
                      ...
                      borderRadius: 1,
                      cursor: 'pointer',
                      ...(updateAcc.currency === 'usd'
                        ? { ...bgColors.primaryLight }
                        : { backgroundColor: 'action.hover' }),
                      border: theme =>
                        `1px solid ${updateAcc.currency === 'usd' ? theme.palette.primary.main : theme.palette.divider}`
                    }}
                  >
                    <Box sx={{ mb: 1, display: 'flex', alignItems: 'center', '& svg': { mr: 2 } }}>
                      <Typography
                        variant='h6'
                        sx={{ ...(updateAcc.currency === 'usd' ? { color: 'primary.main' } : {}) }}
                      >
                        USD
                      </Typography>
                    </Box>
                ...
                <Grid item sm={2} xs={12}>
                  <Box
                    ...
                    sx={{
                      ...
                      borderRadius: 1,
                      cursor: 'pointer',
                      ...(updateAcc.currency === 'eur'
                        ? { ...bgColors.primaryLight }
                        : { backgroundColor: 'action.hover' }),
                      border: theme =>
                        `1px solid ${updateAcc.currency === 'eur' ? theme.palette.primary.main : theme.palette.divider}`
                    }}
                  >
                    <Box sx={{ mb: 1, display: 'flex', alignItems: 'center', '& svg': { mr: 2 } }}>
                      <Typography
                        variant='h6'
                        sx={{ ...(updateAcc.currency === 'eur' ? { color: 'primary.main' } : {}) }}
                      >
                        EUR
                      </Typography>
                    </Box>
                ...
                <Grid item sm={2} xs={12}>
                  <Box
                    ...
                    sx={{
                      ...
                      borderRadius: 1,
                      cursor: 'pointer',
                      ...(updateAcc.currency === 'try'
                        ? { ...bgColors.primaryLight }
                        : { backgroundColor: 'action.hover' }),
                      border: theme =>
                        `1px solid ${updateAcc.currency === 'try' ? theme.palette.primary.main : theme.palette.divider}`
                    }}
                  >
                    <Box sx={{ mb: 1, display: 'flex', alignItems: 'center', '& svg': { mr: 2 } }}>
                      <Typography
                        variant='h6'
                        sx={{ ...(updateAcc.currency === 'try' ? { color: 'primary.main' } : {}) }}
                      >
                        TRY
                      </Typography>
                    </Box>
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    label='Facebook Page link1'
                    placeholder='If you don`t have facebook pages then reach out to us'
                    value={updateAcc.facebookOne}
                    onChange={e =>
                      setUpdateAcc({
                        ...updateAcc,
                        facebookOne: e.target.value
                      })
                    }
                    required
                  />
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    label='Facebook Page link1'
                    placeholder='If you don`t have facebook pages then reach out to us'
                    value={updateAcc.facebookTwo}
                    onChange={e =>
                      setUpdateAcc({
                        ...updateAcc,
                        facebookTwo: e.target.value
                      })
                    }
                    required
                  />
          ...
          >
            <DialogContent>
              <DialogContentText id='alert-dialog-description'>
                Are you sure to remove this Ad account?
              </DialogContentText>
            </DialogContent>
            <DialogActions>
              ...
              <Button variant='contained' onClick={() => deleteAcc(seletedAcc)}>
                Agree
              </Button>
            </DialogActions>
          </Dialog>
